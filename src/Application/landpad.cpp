...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>回调函数<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
void relative_position_cb(const ar_track_alvar_msgs::AlvarMarkers::ConstPtr &msg)
{
    for(auto &item : msg->markers)
    {
        if(item.id == 4.0)
        {
            relative_position.x = -item.pose.pose.position.x;
            relative_position.y = item.pose.pose.position.y;
            relative_position.z = item.pose.pose.position.z;
            relative_yaw = item.pose.pose.orientation.w;
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    //ros::Subscriber relative_pos_sub = nh.subscribe<const ar_track_alvar_msgs::AlvarMarkers::ConstPtr>("/ar_pe_marker", 10, relative_position_cb);
    ros::Subscriber relative_pos_sub = nh.subscribe("/ar_pose_marker", 1, relative_position_cb);
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>参数读取<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    nh.param<float>("kpz_land", kpz_land, 0.4);
    ...
    //允许降落最大高度阈值
    nh.param<float>("land_max_z", land_max_z, 0.5);

    //允许飞行最低高度[这个高度是指降落板上方的相对高度]
    nh.param<float>("fly_min_z", fly_min_z, 0.6);
...
void track_land()
{
    ...
    Command_now.vel_sp[2] =  - kpz_land * relative_position.z;
